...
final class FromOpenApiSpecCommandArgumentValueResolver implements ArgumentValueResolverInterface
{
    ...
    public function resolve(Request $request, ArgumentMetadata $argument): Generator
    {
        ...
        if (!empty($request->getContent())) {
            $payload = array_merge($request->toArray(), $payload);
        }

        if ($request->attributes->has('_route_params')) {
            foreach ($request->attributes->get('_route_params') as $key => $value) {
                if (array_key_exists($key, $payload)) {
                    throw new FailedToCreateObjectFromOpenApiSpec(sprintf('Duplicate key in payload and route parameters: `%s`', $key));
                }
                $payload[$key] = $value;
            }
        }
